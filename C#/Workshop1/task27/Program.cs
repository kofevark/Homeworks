...
// Переменную i тоже обявил прям здесь. Ее значение сейчас i=0
{
    int sum = 0; // Переменнная, куда постепенно складываем результат
    for (; i > 0; i = i / 10) { sum = sum + i % 10; } //Решил использовать цикл for
    // (хотя изначально был while) в виде эксперимента и для сокращения строк кода
    return sum;
}